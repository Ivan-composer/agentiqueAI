...
export default function AISearchPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) {
      router.push(`/ai-search/loading?q=${encodeURIComponent(searchQuery)}`)
    }
  }
    ...
    <main className="min-h-screen flex flex-col items-center bg-gradient-to-b from-white via-[#E5F7FF]/20 to-white px-4">
      <div className="flex-1 w-full max-w-md flex flex-col items-center justify-center gap-8 -mt-20">
        ...
        <form onSubmit={handleSubmit} className="w-full relative drop-shadow-lg">
          <div
            className="absolute inset-0 rounded-[100px] bg-gradient-to-r from-[#08C6C9] to-[#0098EA] opacity-50"
            style={{ padding: '1.5px' }}
          >
